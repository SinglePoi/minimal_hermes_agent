...
本项目不再坚持“零依赖”：当能力明显受益于成熟包时允许引入第三方依赖，并写入
`requirements.txt`、尽量与 Hermes 依赖选型一致。下文中“零依赖 / 零构建”仅表示
该模块当前没有额外依赖或构建步骤，不是全局禁令。
...
20. **前端 Web 页面（web/ 静态站点）**：对齐 Hermes dashboard 的交互契约（参考
    ...
    - 视觉（2026-08-12 按 ui-ux-pro-max 设计系统 Cyberpunk 风格重做）：深空黑
      (#020617) 底色 + 青色霓虹网格/CRT 扫描线 + 终端绿 (#22C55E) CTA 与辉光、
      Fira Code 终端字体；侧栏与主内容为同款深空玻璃面板（blur 20px + 霓虹描边 +
      悬浮阴影）、建议卡片/输入卡/弹窗同步暗色化，emoji 图标全部替换为 SVG；
      补齐 :focus-visible 焦点环与 prefers-reduced-motion 无障碍支持；
      隐藏滚动条（保留滚动功能）
