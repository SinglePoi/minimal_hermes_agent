...
## 约定：依赖策略（2026-08-13 更新）

骨架**不坚持零依赖**。当某个能力明显受益于成熟包时，允许引入第三方依赖
（例如后续 ACP 接入使用 `agent-client-protocol`），不必为“零依赖”而自造轮子。

约束仍要遵守：

1. 优先使用 Python 标准库或项目已引入的依赖，能不加就不加
2. 确需新增时，写入 `requirements.txt`，并尽量与 Hermes 的依赖选型保持一致
3. 既有 README/HANDOFF 中的“零依赖”描述（如 `read_extract.py`、`server.py`、`web/`）
   只表示“该模块当前未引入额外依赖”，不再作为全局禁令
